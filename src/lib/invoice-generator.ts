...
// Helper function to add a single invoice page to an existing document
const addInvoicePage = (doc: jsPDF, data: InvoiceData, pageWidth: number, pageHeight: number) => {
  const margin = 15;
  let yPos = margin;
...
export const generateAllInvoices = (invoices: InvoiceData[]) => {
  ...
  addInvoicePage(doc, invoices[0], pageWidth, pageHeight);

  // Add remaining invoices as new pages
  for (let i = 1; i < invoices.length; i++) {
    doc.addPage();
    addInvoicePage(doc, invoices[i], pageWidth, pageHeight);
  }

  // Save combined PDF
  const fileName = `All_Invoices_${new Date().toISOString().split('T')[0]}.pdf`;
  doc.save(fileName);
};
