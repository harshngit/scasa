...
export default function EditComplaint() {
  ...
  const fetchComplaint = async () => {
    ...
    try {
      ...
      if (data) {
        setFormData({
          complainerName: data.complainer_name || '',
          phoneNumber: data.phone_number || '',
          email: data.email || '',
          flatNumber: data.flat_number || '',
          wing: data.wing || '',
          complaintText: data.complaint_text || '',
          complaintDate: data.complaint_date || new Date().toISOString().split('T')[0],
        });
      }
    } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Navigate back to complaints page after successful update
      setTimeout(() => {
        navigate('/complaints');
      }, 1000);
    } catch (error: any) {
      console.error('Error updating complaint:', error);
      toast.dismiss(loadingToast);
      toast.error(error.message || 'Failed to update complaint. Please try again.');
    } finally {
      setIsSubmitting(false);
    }
  };
