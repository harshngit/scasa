...
import { ArrowLeft, HandHeart, Loader2, User, X, Upload, Building2, Edit } from 'lucide-react';
...
export default function EditHelper() {
    ...
    try {
      ...
      if (data) {
        setFormData({
          ...
          helperWork: data.helper_work || '',
          wing: data.wing || '',
          secretary: data.secretary || '',
        });

        if (data.photo_url) {
          setHelperPhoto(data.photo_url);
          setExistingPhotoUrl(data.photo_url);
        }

        // Reconstruct selected flats from flat_details if available
        if (data.helper_type === 'Home' && data.flat_details && Array.isArray(data.flat_details)) {
          const flats: SelectedFlat[] = data.flat_details.map((flat: any, idx: number) => ({
            flatId: `existing-${idx}`, // Temporary ID since we don't have resident IDs
            flatNumber: flat.flat_number || '',
            ownerName: flat.owner_name || '',
            ownerPhone: flat.owner_phone || '',
            renterName: flat.renter_name || null,
            renterPhone: flat.renter_phone || null,
            residencyType: flat.residency_type || 'owner-living',
          }));
  ...
  const fetchFlats = async () => {
    try {
      ...
      // Match existing selected flats with fetched flats
      if (selectedFlats.length > 0) {
        const matchedFlats: SelectedFlat[] = [];
        selectedFlats.forEach((selectedFlat) => {
          const matchedFlat = data?.find(
            (flat) => flat.flat_number === selectedFlat.flatNumber
          );
          if (matchedFlat) {
            matchedFlats.push({
              flatId: matchedFlat.id,
              flatNumber: matchedFlat.flat_number,
              ownerName: matchedFlat.owner_name,
              ownerPhone: matchedFlat.phone_number,
              renterName: matchedFlat.current_renter_name,
              renterPhone: matchedFlat.current_renter_phone,
              residencyType: matchedFlat.residency_type,
            });
          } else {
            // Keep the existing flat even if not found (might have been deleted)
            matchedFlats.push(selectedFlat);
          }
        });
        setSelectedFlats(matchedFlats);
      }
    } catch (error: any) {
  ...
  const uploadPhotoToStorage = async (): Promise<string | null> => {
    // If no new photo file, return existing photo URL
    if (!photoFile) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const loadingToast = toast.loading('Updating helper...');
    ...
    try {
      ...
      if (error) {
        ...
        toast.error(`Failed to update helper: ${error.message}`);
      ...
      // Navigate back to helper detail page
      setTimeout(() => {
        navigate(`/helpers/${id}`);
      }, 1000);
    } catch (error: any) {
      console.error('Error updating helper:', error);
      toast.dismiss(loadingToast);
      toast.error(error.message || 'Failed to update helper. Please try again.');
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex items-center space-x-4">
          <Button variant="outline" size="sm" onClick={() => navigate(`/helpers/${id}`)}>
            <ArrowLeft className="h-4 w-4 mr-2" />
            Back to Helper
          </Button>
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Edit Helper</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end space-x-4">
            <Button type="button" variant="outline" onClick={() => navigate(`/helpers/${id}`)} disabled={isSubmitting}>
              Cancel
            </Button>
