...
export default function Permissions() {
  ...
  const filteredPermissions = permissions.filter(permission => {
    ...
    return (
      permission.resident_name.toLowerCase().includes(query) ||
      (permission.phone_number && permission.phone_number.includes(query)) ||
      (permission.email && permission.email.toLowerCase().includes(query)) ||
      (permission.flat_number && permission.flat_number.toLowerCase().includes(query)) ||
      (permission.wing && permission.wing.toLowerCase().includes(query)) ||
      (permission.permission_text && permission.permission_text.toLowerCase().includes(query))
    );
  });

  const stats = {
    total: permissions.length,
    thisMonth: permissions.filter(p => {
      const permissionDate = new Date(p.permission_date);
      const now = new Date();
      return permissionDate.getMonth() === now.getMonth() && 
             permissionDate.getFullYear() === now.getFullYear();
    }).length,
    pending: permissions.length, // You can add status field later if needed
  };
            ...
            {isAdminOrReceptionist() && (
              <div className="ml-6 flex gap-3 animate-slide-in-right">
                ...
                >
                  ...
                  <span className="font-semibold">Add Permission</span>
                </Button>
              </div>
            )}
          </div>
        ...
        <Card className="border-0 shadow-lg bg-gradient-to-br from-white to-gray-50/50 dark:from-gray-900 dark:to-gray-800/50">
          <CardContent className="pt-6">
            <div className="relative">
              ...
              <Input
                placeholder="Search permissions by name, phone, email, flat, wing, or permission text..."
                value={searchTerm}
          ...
          <CardContent className="p-0">
            {isLoading ? (
              <div className="flex items-center justify-center py-16">
                ...
                <span className="ml-3 text-gray-600 dark:text-gray-400 font-medium">Loading permissions...</span>
              </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gradient-to-r from-gray-50 to-purple-50/30 dark:from-gray-800 dark:to-purple-950/20 border-b border-gray-200 dark:border-gray-800">
                      ...
                      <TableHead className="font-semibold text-gray-900 dark:text-gray-100 py-4">Permission</TableHead>
                    ...
                    {filteredPermissions.map((permission, idx) => (
                      ...
                      >
                        <TableCell className="py-4">
                          <span className="font-semibold text-gray-900 dark:text-gray-100">{permission.resident_name}</span>
                        </TableCell>
                        <TableCell className="py-4">
                          <div className="space-y-1">
                            {permission.phone_number && (
                              <div className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                                ...
                                <span className="text-sm">{permission.phone_number}</span>
                              </div>
                            )}
                            {permission.email && (
                              <div className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                                <Mail className="h-3.5 w-3.5 text-gray-400" />
                                <span className="text-sm">{permission.email}</span>
                              </div>
                            )}
                            {!permission.phone_number && !permission.email && (
                        ...
                        <TableCell className="py-4">
                          <div className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                            ...
                            <span className="text-sm">{formatDate(permission.permission_date)}</span>
                          </div>
                        </TableCell>
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                      This action cannot be undone. This will permanently delete the permission from
                                      <strong> {permission.resident_name}</strong>.
                                    </AlertDialogDescription>
                                  </AlertDialogHeader>
                                  <AlertDialogFooter>
